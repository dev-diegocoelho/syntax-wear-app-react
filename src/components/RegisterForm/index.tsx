    ...
    return (
        <form className="text-black flex flex-col gap-4">
            ...
            <div>
                ...
                <input className={`w-full border rounded-xs px-1 mt-1 focus:outline-none focus:ring-2 ${errors.cellphone ? 'border-red-500 focus:ring-red-400' : 'border-gray-300 focus:ring-[#5433eb]'} `} type="tel" {...register("cellphone")} />
                {errors.cellphone && <p className="text-xs text-red-600 mt-1">{errors.cellphone.message}</p>}
            </div>

            <button disabled={isSubmitting} className="bg-[#5433eb] text-white font-semibold uppercase rounded-md py-3 transition-all hover:bg-[#4028c7] disabled:opacity-50 cursor-pointer">{isSubmitting ? 'Enviando...' : 'Enviar'}</button>
